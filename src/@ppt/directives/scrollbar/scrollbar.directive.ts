...
import { ScrollbarGeometry, ScrollbarPosition } from '@ppt/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[pptScrollbar]',
    exportAs: 'pptScrollbar'
})
export class PptScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_pptScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() pptScrollbar: boolean = true;
    @Input() pptScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'pptScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.pptScrollbar = coerceBooleanProperty(changes.pptScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.pptScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'pptScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.pptScrollbarOptions.currentValue);

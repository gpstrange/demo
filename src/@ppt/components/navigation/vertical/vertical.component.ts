...
import { PptNavigationItem, PptVerticalNavigationAppearance, PptVerticalNavigationMode, PptVerticalNavigationPosition } from '@ppt/components/navigation/navigation.types';
...
@Component({
    selector       : 'ppt-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : pptAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'pptVerticalNavigation'
})
export class PptVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: PptVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: PptVerticalNavigationMode = 'side';
    @Input() name: string = this._pptUtilsService.randomId();
    @Input() navigation: PptNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: PptVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<PptVerticalNavigationAppearance> = new EventEmitter<PptVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<PptVerticalNavigationMode> = new EventEmitter<PptVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<PptVerticalNavigationPosition> = new EventEmitter<PptVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<PptNavigationItem> = new ReplaySubject<PptNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<PptNavigationItem> = new ReplaySubject<PptNavigationItem>(1);
    ...
    private _pptScrollbarDirectives!: QueryList<PptScrollbarDirective>;
    private _pptScrollbarDirectivesSubscription: Subscription;
    ...
    {
        return {
            'ppt-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`ppt-vertical-navigation-appearance-${this.appearance}`]: true,
            'ppt-vertical-navigation-hover'                          : this._hovered,
            'ppt-vertical-navigation-inner'                          : this.inner,
            'ppt-vertical-navigation-mode-over'                      : this.mode === 'over',
            'ppt-vertical-navigation-mode-side'                      : this.mode === 'side',
            'ppt-vertical-navigation-opened'                         : this.opened,
            'ppt-vertical-navigation-position-left'                  : this.position === 'left',
            'ppt-vertical-navigation-position-right'                 : this.position === 'right'
        };
    }
    ...
    /**
     * Setter for pptScrollbarDirectives
     */
    @ViewChildren(PptScrollbarDirective)
    set pptScrollbarDirectives(pptScrollbarDirectives: QueryList<PptScrollbarDirective>)
    {
        // Store the directives
        this._pptScrollbarDirectives = pptScrollbarDirectives;

        // Return if there are no directives
        if ( pptScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._pptScrollbarDirectivesSubscription )
        {
            this._pptScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._pptScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    pptScrollbarDirectives.forEach((pptScrollbarDirective) => {
                        pptScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.ppt-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._pptScrollbarDirectives.forEach((pptScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !pptScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    pptScrollbarDirective.scrollToElement('.ppt-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: PptNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: PptNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('ppt-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('ppt-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('ppt-vertical-navigation-aside-overlay');

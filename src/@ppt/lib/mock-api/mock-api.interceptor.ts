...
import { PptMockApiService } from '@ppt/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class PptMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(PPT_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _pptMockApiService: PptMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._pptMockApiService.findHandler(request.method.toUpperCase(), request.url);

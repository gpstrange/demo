...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--ppt-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--ppt-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--ppt-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--ppt-${e(key)}`, value], [`--ppt-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`ppt.customProps.background.${colorScheme}`);
            const foreground = theme(`ppt.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ppt-${e(key)}`, value], [`--ppt-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ppt-${e(key)}`, value], [`--ppt-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
